...
def valid_context():
    ...
    work_order = WorkOrder(request_id="applier_unit_test", blueprint_name="test_blueprint", 
                           profile_name="test_profile", order_type="create", goal="test", payload={})
    profile = ProductionProfile()
    project_context = ProjectContext(root_path=".", source_roots=[], test_roots=[])
    
    return ProductionContext(blueprint=blueprint, work_order=work_order, profile=profile, project=project_context)

def test_output_applier_happy_path(valid_context, mocker):
    mocker.patch("pathlib.Path.exists", return_value=True)
    mocker.patch("pathlib.Path.read_text", return_value="### FILE: output.txt\n```python\nprint('Hello World')\n```")
