...
- the Existing Code Context as the primary source for realistic payload content, read_files, and behavioral expectations
...
# Suggested Files
These file suggestions were derived from project analysis and should be used unless the code context clearly proves they are unnecessary.
...
- Use request_id equal to the atomic task id unless there is a strong reason to derive a more specific id.
- Use profile_name: default unless the atomic task explicitly requires another profile.
- Use order_type: create unless the task clearly describes a modification of an existing writable target.
- The proposal must be fully consistent with the blueprint constraints and quality requirements.
...
- Prefer the suggested read_files and writable_files when they match the code context.
- Do not invent unrelated files.
- Use the code context to infer realistic responsibility, definition_contract, happy_path_behavior, and error_behavior.
- Do not use generic placeholder values such as 'Developer', 'Tests the class functionalities', or similarly vague text.
- If this is a unit test blueprint, do not propose production source files as writable output unless the atomic task explicitly requests it.
- Keep the proposal focused and minimal.
- Return YAML only.
- Do not include markdown fences.
- Do not include explanations.
- The acceptance_criteria section must explicitly include every atomic task acceptance criterion, preserving their meaning.
- Every field listed under Blueprint Required Fields must appear in the payload section.
- Do not omit any required payload field, even if the value is already implied elsewhere.
- If the generated test code needs to simulate FILE block response content, build that content from string fragments or line joins inside Python code.
- Do not embed nested raw FILE block fixtures in a way that could be interpreted as additional outer output blocks.
...
- Every blueprint required field is present in payload exactly once when applicable.
"""
