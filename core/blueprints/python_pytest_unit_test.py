...
def build_python_pytest_unit_test_blueprint() -> Blueprint:
    return Blueprint(
        ...
        constraints=[
            ...
            "Do not add external dependencies other than pytest and Python standard library modules",
            "Do not use vague placeholder descriptions in payload fields",
            "When the target behavior depends on structured text input, preserve the exact required text format in test fixtures",
            "Do not indent multiline sample content when line-start markers are part of the contract being tested",
            "When testing file-system behavior, create required directories before writing files or use pytest tmp_path",
            "Construct valid domain objects or minimal compatible test doubles for the target contract",
            "Enum-typed fields must use valid enum values, not placeholder strings",
            "When tests need sample content that mimics the LLM FILE block format, construct that content at runtime from string fragments",
            "Do not embed raw nested FILE block fixtures in a way that can be confused with the outer response format",
        ],
        quality_requirements=[
            ...
            "Tests must assert meaningful behavioral outcomes, not only object existence",
            "Tests must use realistic inputs derived from the component contract",
            "If the component parses structured text, the tests must provide format-valid sample content",
            "If the component interacts with the file system, the tests must handle paths and directories robustly",
            "The tests must avoid invalid placeholder values for typed fields",
            "Tests for parsers of FILE block responses must build sample response content safely without colliding with the outer generation format",
        ],
        default_invariants=[
